defmodule SbgInv.Web.ScenarioView do
  ...
  defp massage_resources(scenario_resources) do
    %{
      ...
      cheatsheet:       Enum.filter(scenario_resources, &(&1.resource_type == :cheatsheet)),
      video_replay:     Enum.filter(scenario_resources, &(&1.resource_type == :video_replay)),
      web_replay:       Enum.filter(scenario_resources, &(&1.resource_type == :web_replay)),
      terrain_building: Enum.filter(scenario_resources, &(&1.resource_type == :terrain_building)),
      podcast:          Enum.filter(scenario_resources, &(&1.resource_type == :podcast)),
